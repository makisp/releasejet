...
- **`includeAiSummary` field on Slack/Discord/Teams notification channels** (boolean, optional, default `true`). When the M3b AI release overview is populated and the flag isn't explicitly `false`, Pro v1.9.0+ renders the summary in default cards. Schema-only change in core; runtime rendering lives in Pro. Webhook channels reject the flag (their envelope contract is independent).

## [1.22.0] - 2026-05-08

### Added

- **config:** new `excludeLabels` option. Issues carrying any listed label are
  dropped from release notes before categorization. Exclude wins on collision
  with `categories`; excluded issues do not trigger `uncategorized: strict`
  failures. CLI prints a summary line listing the matched labels:
  `Excluded 3 issues by excludeLabels filter (internal, chore)`.
- **init:** wizard prompts for internal labels with default `internal,chore`.
- **types:** `CategorizedIssues.excluded: Issue[]` (additive; consumers other
  than the new CLI summary line are unaffected).
...
- F3 — Jira issue linking. New `jira:` config block (`baseUrl` + `projects`)
  detects Jira ticket IDs in issue/PR titles and bodies and renders them
  as inline links next to each issue line in the default template. No
  Jira API calls; project-key allowlist prevents false positives.
- `init` wizard now offers Jira ticket linking setup after the
  contributors prompt.
